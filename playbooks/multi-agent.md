...
Copy every step into the todolist before reasoning. Run the steps in order, because each later step reads an answer an earlier step recorded. A step that does not apply stays in the todolist with a one-line skip reason, so a reviewer can see what was deliberately not done. When a step check fails, open the named cluster in `lessons/10-multi-agent.md`, repair the design, then continue. Symptom-first lookup lives in `references/failure-modes.md`.

## Step 0. Name the data shape

Fix the canonical shape before any orchestration code exists. The shape for this playbook is an information-gain gate plus a two-axis record, where axis one is shared versus isolated context and axis two is topology. Look it up in `references/data-shapes.md`. Every later decision about communication mechanism, file layout, and termination policy derives from those two axis answers, so an unnamed shape leaves the rest of the review with nothing to check against.
...
## Step 1. Gate the added agent on new information
...
## Step 2. Price the collaboration before approving it
...
## Step 3. Fix axis one, inherited trajectory or independent contexts
...
## Step 4. Fix axis two, peer or manager or decentralized
...
## Step 5. Declare the file system mount table
...
## Step 6. Stand up the control plane
...
## Step 7. Define the handoff schema
...
## Step 10. Add fault tolerance for plausible wrong output
...
## Step 12. Cross a trust boundary only with the standard protocol
...
## Step 13. Measure per-role justification and remove what fails
...
## Step 14. Track comprehension debt
