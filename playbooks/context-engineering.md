...
Lookup the row for this playbook in `references/data-shapes.md`.
...
If the hash drifts mid-session, open `lessons/02-context-engineering.md`.
...
If a second path concatenates prompts, open `lessons/02-context-engineering.md`.
...
If a clock or plan string sits in the system line, open `lessons/02-context-engineering.md`.
...
If the always-on list grows or sorts by recency, open `lessons/02-context-engineering.md`.
...
If a needed skill never loads, or a body sits in the frozen system line, open `lessons/02-context-engineering.md`.
...
If the loop can run forever or rereads one path, open `lessons/02-context-engineering.md`.
...
If a webpage or PDF can rewrite the next action, open `lessons/02-context-engineering.md`.
...
If a one-shot model tally writes the bar, open `lessons/02-context-engineering.md`.
...
If last-N deletion is the history policy, open `lessons/02-context-engineering.md`.
...
If tens of thousands of search tokens land in the main trace, open `lessons/02-context-engineering.md`.
...
If a gold id vanishes from a note, open `lessons/02-context-engineering.md`.
...
If a prompt change ships without those two checks, open `lessons/02-context-engineering.md`.
...
- `playbooks/memory-knowledge.md` when the missing store is cross-session user memory or a shared knowledge plane.
- `playbooks/tools.md` when the defect is schema form, disclosure policy, or a minted tool rather than window layout.
