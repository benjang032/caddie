...
- **Answer-only:** A casual or formula-only definition stays in this file; do not open a lesson or playbook, or create a working checklist. For a curriculum-backed, scoped technical explanation or comparison, open only the matching lesson cluster or clusters below. Do not open a playbook or turn its steps into a checklist.
- **Implementation or review:** Match the task to a playbook below. Open that file and make a working checklist from its steps before reasoning. Use the host's task or todo list when available. Read a lesson file only when the playbook names a cluster.
...
Multi-topic answer-only questions may open the matching lesson clusters, still without a playbook or working checklist. A casual or formula-only definition stays on this file.
