...
Make a working checklist from these steps before reasoning. Use the host's task or todo list when available. Name the data shape before any loop or tool change. If a check fails, open [lesson 06](../lessons/06-interaction.md) and the named cluster `source-ids`.
