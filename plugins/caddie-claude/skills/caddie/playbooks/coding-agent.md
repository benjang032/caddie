...
Make a working checklist from these steps before reasoning about the agent. Use the host's task or todo list when available. Work them in order. A step that does not apply stays in the list with its skip reason recorded, because a silent omission reads later as a completed check.
